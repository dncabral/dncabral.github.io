 ...
 */
function getTopText () {
    $message = "Recebeu uma nova mensagem enviada através do formulário de contacto no diogocabral.net.";
    $message = strlen($message) > 0 ? $message : "You received a new message sent via the contact form on diogocabral.net.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "diogocabral.net"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 119.0.19",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "diogocabral.net") {
